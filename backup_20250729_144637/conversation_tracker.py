...
class ConversationTracker:
    ...
    def get_all_topics(self) -> List[str]:
        ...
        try:
            # Get unique topics using facets
            results = self.search_client.search(
                search_text="*",
                filter=f"thread_id eq '{self.thread_id}' and recordType eq 'conversation_turn'",
                facets=["topic"],
            ...
            topics = []
            if hasattr(results, 'facets') and 'topic' in results.facets:
                for facet in results.facets['topic']:
                    topics.append(facet.value)
                    self._topics_cache.add(facet.value)
